...
| **`abort_attempted`**      | After every `abort_agent_session()` (`sessions.steer`) call (retry-start + inline + **escalation** + **reviewer_retry**) | `{session_key, result, agent_role, reason, source}` (`source` ∈ `retry_start`/`escalation`/`reviewer_retry`; the `halt_message_sent` field was removed with the halt fallback) | 6.1.b |
| **`abort_verify_failed`**  | When `verify_session_stopped()` returns `False` (no longer halts — soft-continue; can false-fire in the rare zombie case because the steer spawns a brief stop-turn) | `{session_key, stamp_path, agent_role, reason, source}` (`source` ∈ `retry_start`/`escalation`/`reviewer_retry`) | 6.1.b |
...
**Executor retry and OpenClaw:** When `executor_retries` > 0 and the orchestrator is about to invoke attempt N+1, it first calls `abort_agent_session()` (best-effort, with the 3x retry loop described above; `sessions.steer` interrupt) to stop attempt N in the OpenClaw gateway, so the prior run does not keep streaming or refreshing `executor_activity.stamp` after the orchestrator has moved on. If the abort succeeds but `verify_session_stopped` reports the stamp is still advancing, the orchestrator emits `abort_verify_failed` and launches attempt N+1 anyway (soft-continue, same contract as `_handle_stall_outcome`).
