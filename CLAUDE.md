...
`GET /api/queue/{id}/snapshot` carries the same `cost_total`/`tokens_total`/`duration_seconds` for the row expansion, plus **`metrics_phases`** (METRICS-E3) — a compact per-phase projection (`{phase, duration_seconds, executor_attempts, escalations, cost_total, tokens_total, tokens_breakdown}`, built by `_compact_metrics_phases`, same keys as the `/api/metrics-summary` per-phase rows) behind the chip-opened breakout table (`QueueMetricsBreakout` → the shared `PhaseMetricsTable`). COMPLETED rows additionally offer **View report** → `GET /api/queue/{entry_id}/report` (METRICS-E4): the full metrics summary for *that entry's* project (shared `_build_project_metrics_summary`), its `completion_report.md` (`{found, content, mtime}`), and roadmap checkbox counts — rendered in `QueueReportModal` through the Monitor's `PipelineCompletePanel` (which accepts `phaseCounts` and hides Generate-Docs when no `onGenerateDocs` handler is passed). Per-entry elapsed is client-derived from existing timestamps (running: `started_at`; parked: time waiting since `parked_at`; failed: `failed_at`). The enrichment adds ~2 small file reads per entry per 5s poll — deliberately uncached at current queue sizes.
