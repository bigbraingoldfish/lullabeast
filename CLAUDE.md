...
- `cost_total` / `duration_seconds` — aggregated from the entry's own `{project}/.autodev/pipeline/metrics.jsonl` by the shared pure helper **`_project_metrics_totals`** (read + dedup-keep-last-per-phase + sums — the same math `/api/metrics-summary` delegates to; duration is per-phase **active work** summed, never run_summary calendar wall-clock).
...
`GET /api/queue/{id}/snapshot` carries the same `cost_total`/`duration_seconds` for the row expansion. Per-entry elapsed is client-derived from existing timestamps (running: `started_at`; parked: time waiting since `parked_at`; failed: `failed_at`). The enrichment adds ~2 small file reads per entry per 5s poll — deliberately uncached at current queue sizes.
