...
The orchestrator's `_queue_preflight()` checks: directory exists, `.git` present, `roadmap*.md` present, and **(PREREQ-3) every declared *required* tool is present on the host**. This is intentionally lighter than the server's `_run_preflight_checks()`, which also validates symlink, `.gitignore`, agent workspace files, etc. **Known MVP limitation:** a project that passes `_queue_preflight` may still fail mid-pipeline if the full server-side preconditions are not met. The server's `_run_preflight_checks` is used at queue-add time and at `trigger-next` time; the orchestrator's check runs only on auto-advance between queue entries.

**PREREQ-3 declared-tool re-probe.** When the project's `verification.md` carries a `## Prerequisites` block, `_queue_preflight` reads the declared tool **names** (`prereq_spec.parse_prerequisites`) and probes each on the host (`host_probes.probe`); a definitively `missing` tool returns `(False, "missing required tool: <name>")` — failing fast on auto-advance before the orchestrator burns executor attempts on an unsatisfiable phase (the `baseball` `INFRA-E1` incident). An inconclusive `unknown` probe does **not** block (a false negative must never strand work); env keys are **never** read here (no value ever reaches the orchestrator). The probe is deterministic and timeout-bounded (`host_probes` never raises/hangs), which is why the method's docstring permits this one bounded subprocess where it otherwise forbids subprocess work. Absent/unreadable `verification.md` ⇒ no declaration to enforce ⇒ behavior identical to pre-feature.

### Prerequisite preflight rows + `scaffold-env` (PREREQ-3, server)

The two PREREQ foundations — `autodev/pipeline/prereq_spec.py` (parser) and `autodev/pipeline/host_probes.py` (host probes) — are wired into the server preflight by `_prerequisite_check_rows(ver_text, repo_path)` (`ui/server.py`), called from `_run_preflight_checks` reusing the `verification.md` text the verification-doc check already reads. It appends one row per declared prerequisite, in the standard `{check, status, message}` shape **plus** additive `{prereq: "tool"|"env", required: bool, guidance?, kind?}`:
- **Tool rows** — `probe(name)`: `found`→`pass`, `missing`→`fail` (+`guidance`), `unknown`→`warn`. **Every declared tool is required** — the Tools schema has no advisory marker (template glossary: *"Required tools are probed at Preflight; a missing one blocks Launch"*); the non-blocking "advisory" path is the inconclusive `unknown` outcome (`DEC-4`).
- **Env rows** — presence-only via `_env_key_presence(env_path, names)`, three states: `pass` ("set"), `warn` ("in .env, empty"), `warn` ("not yet"). **Never a value** — the helper inspects the text after `=` solely for emptiness and returns only the three-state label; no row message ever contains a value. Env keys never block (`required: False`).

`POST /api/setup/preflight` returns an additive top-level **`launch_blocked`** = any *required tool* row is `fail` (env keys and `unknown` probes never contribute). A project with no `## Prerequisites` block contributes zero rows (baseline-identical).

`POST /api/setup/scaffold-env` (`{repo_path}` → `{ok, env: {NAME: presence}, written: [NAMES]}`) writes the declared env-var **names** to the project `.env` as blank `KEY=` lines, each preceded by a `# <purpose>` comment. **Append-only** (create if absent, append only absent keys, never overwrite/reorder an existing line), **atomic** (`_atomic_write_text_file`, mkstemp+`os.replace`), and **value-free** (it writes no value and returns only the value-free presence map). It also ensures `.env` is gitignored (`_ensure_env_gitignored`, append-only). No `verification.md` / no declared env ⇒ no-op (no file created). This realises `DEC-1`/`DEC-2`: Lullabeast scaffolds names, the user fills the values in their own `.env`, which never leave their machine.
