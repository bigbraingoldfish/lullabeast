...
| `AUTODEV_IDEAS_STARTUP_GRACE` | default `30` (seconds) | **Pre-first-activity** wait for the Ideas chat poll: how long the poller tolerates a non-advancing stamp before declaring `PollResult(False, "no_first_activity")`. Catches cold OpenClaw session-creation hangs distinct from mid-turn stalls. |
