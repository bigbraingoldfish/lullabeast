...
- **`install.sh` / OpenClaw:** Step 9 audits **`hooks`** (`enabled`, `hooks.token`, `allowRequestSessionKey`, `allowedSessionKeyPrefixes` including `pipeline:` and `ideas:`), offers an atomic baseline patch, and can generate **`hooks.token`** plus **`AUTODEV_HOOKS_TOKEN`** in `.env` when appropriate. It then handles `tools.profile` and registers pipeline agents (`planner`, `executor`, `reviewer`, `escalation`, `prd-creator`, `roadmap-converter`) / `hooks.allowedAgentIds`. **`hooks.token`** is the webhook Bearer for `/hooks/agent`, not the gateway Control UI token — see [SETUP.md — openclaw.json](SETUP.md#openclawjson-requirements) and [OpenClaw tools docs](https://docs.openclaw.ai/tools).
