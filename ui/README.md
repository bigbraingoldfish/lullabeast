...
- **Activity tab:** Event badges show **human-readable** type labels; hover the badge for the event description; the expanded row shows the raw `event` type and detail. The feed is **curated for signal** (`isFeedVisibleEvent`): `poll_outcome`, `reviewer_verdict`, `reachability_not_applicable`, and no-op `abort_attempted` (`skipped_idle`) rows are hidden — full detail stays on the **Pipeline log** tab. Empty ring buffer: **No events yet. Events appear here as the pipeline runs.**
