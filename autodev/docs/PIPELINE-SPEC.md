...
> **Webhook model field:** By default the orchestrator does **not** send a `model` field on `POST /hooks/agent` for planner, executor, or reviewer, so OpenClaw uses each agent’s configured model from `openclaw.json` (same semantics as the Ideas / prd-creator webhook path). The one exception is the **per-phase model override**: the dashboard writes `{raw_id: {role: model}}` into the project’s `phase_model_overrides.json` (`POST/DELETE /api/phase-model-override`), and the three phase invoke sites thread the current phase’s entry as `model=`. Because the `responseUsage` pre-seed creates the session before the webhook fires, the override also rides that pre-seed `sessions.patch`; a session's model is fixed at creation, so the pre-seed is what binds it when enabled (the webhook `model=` covers the pre-seed-disabled path). The override needs no gateway restart, covers every attempt in that phase (`models_used` stamps the effective model), and is dropped when the phase closes. Setting an override runs a bounded liveness check against a local provider's backing server and warns immediately when it is down or not serving the model; there is no pre-invocation probe at run time, so a server that dies later surfaces as a stall, then an escalation. Durable per-role changes go through the Settings Model roles card or the `*_MODEL` variables; a raw `agents.list[].model` edit is reverted by per-boot reconcile on owned installs.
...
|---|---|---|
| `RETRY` | Resumes the agent that was in-flight when the pipeline stopped — `current_agent` is restored from the `resume_target_agent` stash that `/api/resume-ready` records before routing through escalation (defaults to the planner only when that stash is absent), then transitions to `RUNNING`. Drives the StoppedRecoveryPanel resume flow. | A clean operator stop (StoppedRecoveryPanel "Resume"); recovery from a transient failure (network, timeout, fluke) |
...
All three writers (repo-init dispatch, main dispatch, crash handler) go through the shared `_write_escalation_failed_atomic` helper — mkstemp + `os.replace` per the house atomic-write rule, never-raising (the callers are already on a failure path; a diagnostics-write failure must not derail the `HALTED_SILENT` transition that follows). The crash-handler variant substitutes `exception` / `escalation_error` fields for `gate` / `original_failure_reason`.
...
When a project escalates under an **auto-queue**, the orchestrator parks it (`_queue_park_active_entry("ESCALATION")`) and advances to the next eligible project — after first holding (bounded, `AUTODEV_ESCALATION_SUMMARY_WAIT`) for the escalation agent's `escalation_summary.json`, which must land before the advance repoints the `pipeline-project` symlink (see the *Agent-owned escalation advisory* paragraph under §`phase_state.json`). The operator may **bank** an answer for the parked project at any time via `POST /api/command` with `target_project_path`; the server writes only the per-project `pending_escalation_command.json` (it never writes `pipeline_queue.json`). The loop closes on the next selection:
...
**`queue_mode` is evaluated once, at escalation time (manual = hold; not re-checked while parked).** The park→advance above is gated by `_queue_after_park_maybe_advance()`, which reads `queue_mode` at the instant of escalation — `auto` parks + advances (above); `manual` returns `False` and the orchestrator holds in `WAITING_FOR_HUMAN`, polling for an in-place answer. The check sits inside the escalation dispatch, which runs only while `_should_invoke_escalation_agent()` is true (false once `WAITING_FOR_HUMAN` is set), so a `manual → auto` toggle *after* a project has parked does **not** retroactively advance it, and server-side `_maybe_autostart_queue` won't either (it fires only when the pipeline is idle). The operator must answer or `STOP` the parked escalation; the queue then advances on the next selection. Intended behavior, not a bug (confirmed live 2026-06-08).

**Parked-entry metadata hygiene + parked-target revival routing (Defect C).** Every transition OUT of a parked entry state scrubs the canonical set `{parked_state_snapshot, parked_at, parked_reason, parked_pipeline_status, answered_at}` via `queue_semantics.scrub_parked_fields` (`PARKED_ENTRY_FIELDS`) — the single source shared by the orchestrator (the two selection activations + `_queue_restore_parked_entry_to_active`) and the server (`_queue_mark_matching_entry_active` promote/demote, `_queue_demote_stale_active_entries`); without it a row can land `READY` while still carrying a stale snapshot (the live Minecraft drift). And `POST /api/resume-orchestrator` / `POST /api/setup/switch-project` route a *parked* (`ESCALATION` / `ESCALATION_ANSWERED`) target through the `--revive` path (`_spawn_orchestrator(revive_entry_id=…)` → `apply_cli_revive` → `_select_next_queue_project(target_entry_id=…)`), **skipping** the phase-0 state write + the bare `_queue_mark_matching_entry_active` promote (mirroring `/api/queue/{entry_id}/relaunch`), so the escalated phase + any banked command are restored rather than restarted. A non-parked target keeps the bare-promote path.
...
**`ERR_UNACCOUNTED_DELETION`:** The gate runs `git diff --name-only --diff-filter=D <phase_base_commit> HEAD` in the workspace, plus `git ls-files --deleted` for uncommitted deletions, then cross-references the union against `file_manifest` and the optional `files_deleted` array. Any file that appears in neither list triggers this error. If `phase_base_commit` is absent from `pipeline_state.json`, the gate **fails closed**, returning the stdout string `FAIL` (exit 0) with `last_error_code=ERR_MISSING_BASE_COMMIT` in `phase_state.json` — it does not skip or warn. (The executor gate is a *verdict gate*: it signals via a stdout verdict string + exit 0, never an exit code — see the two-convention "Gate Script Interface Contract" in CLAUDE.md.) Without a base commit reference the deletion check cannot run, and a silent skip would allow MiniMax file-deletion to go undetected. The orchestrator retries with a fresh session on this error. The same fail-closed contract covers the committed-diff `git diff` returning non-zero (`ERR_GIT_DIFF_FAILED`), the working-tree `git ls-files --deleted` probe returning non-zero (`ERR_GIT_DIFF_FAILED`, M4 — a repo lock grabbed between the two probes, or an OOM-killed second process), and the deletion check itself crashing — git missing/killed/timeout (`ERR_DELETION_CHECK_CRASHED`); none may skip-and-PASS. (Older versions of this spec described the absent-`phase_base_commit` case as non-fatal and skipped with a warning — that behaviour was removed; the `ls-files` probe was likewise a silent-skip on non-zero rc until M4 gave it the same fail-closed treatment. See `autodev/tests/test_defensive_c3_07.py` and `autodev/tests/test_executor_gate_unaccounted_deletion.py` for the fail-closed contract.) This catches models that delete files under token pressure and self-report `all_passing: true` — see PIPELINE-CONSTRAINTS.md §2 > MiniMax M2.5 File Deletion Under Token Pressure.
...
All three contract-shape verdicts re-invoke the reviewer in a **fresh session** (`-u{N}` suffix via `_reviewer_session_key`, matching `CONTRACT_FAILURE`'s `-c{N}`) with a verdict-specific instruction delivered as the webhook `message=` via the one-shot `reviewer_retry_directive` field (the dead-write `unverified_instruction` field it replaced was never delivered). The directive is **enriched** with the gate's specific problem list: the gate stashes its `_check_*` problems in `phase_state.reviewer_unverified_detail`, and `_compose_unverified_directive` appends them so the reviewer is told exactly what failed. Before re-invoking, the orchestrator **interrupts the prior reviewer run** via `_abort_active_agent_session("reviewer_retry")` so a still-streaming embedded run cannot run concurrently or keep writing the shared `reviewer_output.*` files. This routes through the consolidated `_interrupt_agent_session` helper with `skip_if_idle=True`: it skips the steer (a clean `skipped_idle` no-op) only when the prior reviewer's turn has **provably ended** — read from the session transcript's last assistant row (`_agent_turn_still_in_flight`: terminal vs `toolUse`/recoverable-overflow, **not** a stamp-movement window, which is silent for a whole model call) — avoiding a gratuitous interrupt turn on a finished session; a still-in-flight (or unresolvable) reviewer is steered (`sessions.steer`), then the helper waits for the activity stamp to settle before the re-invoke. The reviewer's main retry budget (`reviewer_retries`) is preserved so a legitimate code-quality rejection on the next pass can still drive the 3-pass attribution routing. Pooling replaces the prior per-flavour counters — sprawl is the bug the consolidation eliminates now that a third contract-shape verdict exists.
...
There is no run-time model-health probe and no SSH recovery. Agent/model liveness is owned by the OpenClaw activity-stamp hooks — `startup_grace` / `no_first_activity` for a cold start, `stalled` for a mid-turn death (`poll_for_sentinel`) — which fire identically for cloud and local agents, so a genuinely dead model surfaces as "no activity" and is caught there rather than by a pre-invocation probe. Assignment time is the exception: the dashboard pickers and the phase-override POST run a bounded liveness check against local providers (`GET /api/models/probe`), so a dead server is flagged while the operator is looking instead of burning a startup window.
...
The same "ignore this `.done`, keep waiting" mechanism is extended by an optional `sentinel_acceptor` predicate (Layer 2 — discarded-verdict races). The three phase-agent poll sites pass `Orchestrator._make_verdict_hold_acceptor(role, session_key, attempt_start)`, which HOLDS a fresh `.done` while the session is still alive past it and may yet write the real verdict, for **either** of two reasons: **(1) recoverable context-overflow** (the session's last assistant row is `stopReason:"error"` with a `"Context overflow …"` errorMessage, no verdict on disk) until OpenClaw's compact-and-resume lands the verdict → `sentinel_overflow_hold`; or **(2) the turn is still in flight past `.done`** (the session's last assistant row is a non-terminal tool-loop step, `stopReason: "toolUse"`, so the agent wrote `.done` but is still acting and may yet write its verdict) → `sentinel_verdict_hold`. Terminality drives (2), **not** stamp-vs-`.done` timing: the `.done` write's own `after_tool_call` hook bumps `{role}_activity.stamp` after `.done` even on a genuine no-verdict end, so the stamp would false-hold every real CONTRACT_FAILURE. This closes the race where the gate reads a missing verdict, escalates, and discards the verdict the still-active session produces moments later — and, for (2), the false `CONTRACT_FAILURE` + concurrent-retry that followed. The predicate accepts immediately for a fresh, parseable `{role}_output.json`, a spent hold budget (`AUTODEV_OVERFLOW_HOLD_BUDGET`, default 900 s), or a **terminally-ended turn** (no overflow row **and** the last assistant row is not a `toolUse` step — so a real CONTRACT_FAILURE accepts with no stall-window latency); the hold stays bounded by that budget plus the stall / startup-grace / 75-min backstop timers (a turn that stops touching its session goes silent > stall threshold → abort). A raising acceptor fails open (accepts). The completion-review poll is deliberately not wired.
...
**Agent-owned escalation advisory (2026-06-11; supersedes the orchestrator's LLM advisory).** All three escalation dispatch sites (reviewer/main, repo-init, crash handler) record a **deterministic, factual** `escalation_message` via `_compose_fallback_reason(ps)` (`escalation_advisory_status="fallback"`, helper `_record_escalation_reason`) **before** transitioning to `WAITING_FOR_HUMAN` — the panel shows an honest reason the instant it renders, with no loader and no orchestrator LLM call. The fallback is built from hard signals (`last_error_code`, the honest `escalation_trigger_reason`, and `failure_context.json`'s `failing_agent`/`gate_error_codes`/`attempt_number`) and **never** from the phase's `failure_language` (presenting that expected-failure description as observed reality is the fabrication that produced the misleading "blank white page" message). The escalation **agent** then composes the richer advisory itself (its escalation-summary skill) and writes `escalation_summary.json` BEFORE notifying the operator; the orchestrator clears any stale copy at dispatch (`_clear_stale_escalation_summary`) and promotes a fresh one into `escalation_message` / `escalation_recommended_action` / `status="ready"` (`_promote_agent_escalation_summary`) from the WAITING_FOR_HUMAN poll loop and at resolution — so the dashboard upgrades in place within one poll cycle of the summary landing. The retired `"generating"` status no longer exists: a status only an in-process wait could clear would strand the panel after a queue auto-advance. *Auto-advance hold (closes the symlink write race for the common case):* the main dispatch holds a queue auto-advance until the agent's summary lands — after the webhook returns SUCCESS and before `_queue_after_park_maybe_advance()`, `_wait_for_escalation_summary_before_advance` polls (~2 s) for `escalation_summary.json` and promotes it on appearance (the dispatch just cleared any stale copy, so appearance ⇒ fresh — no mtime guard needed). Bounded by env `AUTODEV_ESCALATION_SUMMARY_WAIT` (default 300 s; `0` disables); a pending STOP breaks the wait early **without consuming** the stop sentinel (the loop-top `_check_stop_requested()` owns consumption). The hold engages only when the advance would actually fire (queue_mode `auto` with a non-empty queue — the same read `_queue_after_park_maybe_advance` performs): in manual / single-project mode the WAITING_FOR_HUMAN poll loop already promotes within one cycle, so a hold there adds nothing. Cost is ~zero wall-clock on this deployment — the escalation agent (qwen3.6-27b) and the pipeline agents (darkqwen3.6-27b-mtp) share one single-GPU llama-swap server, so the next project's planner cannot make token progress during the escalation turn anyway; the budget only bounds a hung agent. The repo-init dispatch (never auto-advances) and the crash handler (terminating) deliberately do not hold. *Remaining limitation — timeout-exceeded only:* if the summary has not landed when the budget expires, the advance proceeds and repoints the `pipeline-project` symlink, so a late write lands in the repointed tree (OpenClaw's write sandbox forces symlink-relative writes) — the parked row keeps the deterministic message; the operator's Signal notification still carries the rich summary (the webhook message points the agent's *reads* at the resolved absolute path). While the advisory is still `"fallback"` and the pipeline is `WAITING_FOR_HUMAN`, the dashboard panel shows a subtle "Escalation agent is reviewing…" pulse cue (a hint that the text may upgrade in place — not a loader; `"generating"` stays retired).
