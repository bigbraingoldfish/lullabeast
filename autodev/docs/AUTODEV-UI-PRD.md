...
**Git checkout recovery** — When `last_action` contains **Git operation failed**, amber strip + **Recover Git** with `title` (stash including untracked, checkout, state update; not `git reset`). **`GET /api/state`** → **`git_recover_suggested_branch`**; modal **Branch to return to** prefilled from config + repo.
...
**Empty roadmap list** — For **`IDLE`** / **`UNKNOWN`** with no roadmap rows, idle-oriented copy + `data-testid="roadmap-empty-idle"`. Otherwise a single generic empty line.
...
Header: **"Escalation — Human Input Required"**. The headline reads in plain English: the LLM advisory summary when `escalation_advisory_status === "ready"`, otherwise the clean, deterministic `escalation_headline` (e.g. *"Phase REND-E1 needs your input"*) served by `GET /api/state`. **P1 Stage G1 (de-blame):** the raw `escalation_trigger_reason` — often internal blame-attribution jargon (e.g. *"Impl blame cap reached (4x): [L3] … defaulting to impl."*) — is **no longer the headline**; it is demoted into the panel's collapsible "Internal reason" disclosure for operators who want the internal detail (preserved, not deleted). **P1 Stage G3 (consolidation):** the escalation/recovery surface is now a **single shared `EscalationCommandPanel` component rendered identically by both the Pipeline Monitor and the Queue view** — the previously-forked Queue render (`QueueActionHub`) was removed, so the headline, the three advisory states (generating / ready / fallback), the "Internal reason" disclosure, the recovery buttons (sourced from one `ESCALATION_CMD_DEFS` constant), and a single canonical fallback string are all defined once. Both views dispatch through `POST /api/command`; the Queue passes the **selected entry's** escalation fields as props and adds `target_project_path` so a command issued for a *parked* (non-active) ESCALATION project is **held** and fires when the orchestrator picks that project back up. The Monitor-only orchestrator-lifecycle controls (Restart Orchestrator, the "command submitted" screen, the orchestrator-down warning) are prop-gated **off** in the Queue. **P1 Stage G3 follow-up (single-source the per-entry snapshot):** the Queue sources **all** escalation fields — the advisory block (headline, summary, suggested action, internal reason, advisory status), the cap counter (`escalation_resets`), and the eligibility probes (`executor_output_exists` / `merge_probe_passed`) — from the per-entry `GET /api/queue/{id}/snapshot` (the *selected* project), not from `GET /api/state` (the *active* symlink project). So a parked entry's panel describes **its own** project — the same project the dispatch targets — instead of the active one's advisory. `/api/state` and the snapshot share one `_compute_escalation_view` server helper, so the two surfaces compute the identical view and cannot drift; the snapshot's live `pipeline_state`-sourced fields (`pipeline_status`, retries, `last_action`) remain `null` for non-active entries.
...
9. **UI-FEED** — Activity feed, synthetic event display, event type badges (labels + raw on hover), live SSE updates, pipeline_events.jsonl upgrade path handling
