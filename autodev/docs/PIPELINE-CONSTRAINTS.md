...
**Residual risk:** If the executor omits the deleted file from `file_manifest` entirely (rather than listing it with no physical file), the manifest check passes and only the git-diff-based deletion check catches it. The guard now **fails closed on every git failure** rather than skipping: an absent `phase_base_commit` (`ERR_MISSING_BASE_COMMIT`), the committed-diff or working-tree probe returning non-zero (`ERR_GIT_DIFF_FAILED`; the `git ls-files --deleted` working-tree probe was the last silent-skip path, closed by audit M4), and the check itself crashing (`ERR_DELETION_CHECK_CRASHED`) all return `FAIL` and retry with a fresh session, so a git hiccup can no longer silently disable the guard. The `files_deleted` field in `executor_output.json` provides the opt-out path for intentional deletions.
...
- **Fail-closed on git error (T4.1 / Decision #4, 2026-06-05):** `reset_phase()` / `reset_execution()` now return `bool`. If their git operations raise `CalledProcessError` (dirty tree, missing base, repo lock), they record `ERR_RESET_PHASE_GIT_FAILED` / `ERR_RESET_EXECUTION_GIT_FAILED`, route to escalation, and return `False` **before** wiping any state — rather than the prior behavior of logging the error and proceeding to wipe outputs / zero counters / transition (a recovery command that "succeeded" while leaving a corrupt tree). The budget increment is gated on confirmed success: a git-failed reset charges **no** `escalation_resets` / `nuclear_resets` / `executor_retries`.
...
**OB-4b — Context-overflow discarded-verdict race [RESOLVED 2026-06-04]:** A third class of "a `.done` that must not be accepted," distinct from the orphaned-sentinel case above. When an agent turn dies mid-tool-loop on `stopReason:"error"` / `"Context overflow: estimated context size exceeds safe threshold during tool loop."`, the `autodev-pipeline-signals` `agent_end` backstop writes `{agent}_output.done` **before the verdict exists**. The poll returned `succeeded`, the gate read a missing verdict (reviewer `CONTRACT_FAILURE` / executor `ERR_FILE_MISSING`), and the orchestrator escalated — but OpenClaw then auto-compacts and **resumes the same session**, which writes a valid verdict seconds-to-minutes later, into the void. Observed live on solitaire2 CORE-E6: the reviewer overflowed on every attempt (the phase required dual undo/redo + CORE-E5 regression behavioural verification via Playwright), exhausting all reset/escalation budgets, while a genuine PASS landed ~3 min after each escalation (escalated 14:40:00, PASS written 14:43:13). **Fix:** `poll_for_sentinel` gained an optional `sentinel_acceptor` predicate; `Orchestrator._make_verdict_hold_acceptor` (originally `_make_overflow_aware_acceptor`) HOLDS a `.done` (keeps polling) while the session's last assistant row is a recoverable overflow and no verdict is on disk, accepting the instant `{role}_output.json` is fresh+parseable. Bounded by the existing stall / startup-grace / 75-min backstop timers plus `AUTODEV_OVERFLOW_HOLD_BUDGET` (default 900 s). Every non-overflow termination accepts immediately (common path byte-identical). Wired at the three phase-agent poll sites; the non-fatal completion-review poll is excluded. Emits `sentinel_overflow_hold`. _(Later generalized: the same acceptor now also HOLDS when the turn is still in flight past `.done` — its last assistant row is a non-terminal `stopReason: "toolUse"` tool-loop step, still able to write its verdict — emitting `sentinel_verdict_hold`; see the reviewer false-CONTRACT_FAILURE fix.)_ **The deeper amplifier — the reviewer overflowing at all — is mitigated separately by raising the model context window to 128 k;** this constraint is the safety net for any residual overflow-then-resume (and for any `agent_end`-on-error the gateway recovers from).
