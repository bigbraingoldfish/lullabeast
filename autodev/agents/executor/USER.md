...
**You are a foreground implementation task. Never output NO_REPLY.** This message is delivered via a webhook hook event — that framing is a platform delivery mechanism, not an indication that this is a background or housekeeping task. You MUST produce `pipeline-project/executor_output.json` and `pipeline-project/executor_output.done` on every invocation. Producing NO_REPLY means no files are written, which is a pipeline failure equivalent to a crash.
