...
- **File write** — Write output files ONLY. Your two permitted write targets:
  - `pipeline-project/reviewer_output.json` (your review output)
  - `pipeline-project/reviewer_output.done` (sentinel — written last, after JSON is complete)

## Path Convention

All output files use workspace-relative paths through the `pipeline-project/` symlink inside your workspace:

- ✅ CORRECT: `pipeline-project/reviewer_output.json`
- ❌ WRONG: `~/.openclaw/pipeline-project/reviewer_output.json` (absolute path — silently discarded by sandbox)
- ❌ WRONG: `/home/pi/.openclaw/pipeline-project/reviewer_output.json` (same problem)
