...
- **M4 follow-up — Broken project path recovery**: `GET /api/state` exposes **`project_dir_ok`** / **`project_dir_message`**; friendly multi-line **503** `detail` for missing/dangling `project_dir_path`; **switch-project** allowed while pipeline is active **only** when that configured path is broken (repair via same modal as STOPPED); pipeline monitor unlocks clickable project path + amber banner in that case.
...
- [ ] `UI-E8` | HIGH | Add active preflight validation that auto-resolves what it can and clearly communicates what requires operator action
  > **Update (2026-03):** Implemented extensions — `POST /api/setup/preflight` accepts optional `roadmap_seed` and `prd_content`; writes `roadmap.md` / `prd.md` when valid (fails on conflict with existing files); checks `git` via `git --version`; auto-initializes git when `.git` is missing; removed per-repo `origin` check in favor of system git availability; roadmap seed quality warning row when a seed is sent. `POST /api/setup/launch` accepts optional `prd_content` for Ideas handoff.
