...
- [x] `UI-2` | LOW | Implement the header bar showing the AUTODEV wordmark, pipeline status pill, project path, and liveness dot
  > Test: AUTODEV wordmark renders in JetBrains Mono or Space Mono. Status pill renders correct label and color class per `ui/index.html` `PIPELINE_LIVE_PILL` + `formatWaitForSentinelLabel` for `WAITING_FOR_SENTINEL`: RUNNING (teal `run-pulse` on `bg-[#0d9488]`), WAITING_FOR_SENTINEL (static teal `#0d9488`; `Running` / `Running {Agent}` from `current_agent` on the header; queue list uses `live_current_agent` on the matching entry), WAITING_FOR_HUMAN (`NEEDS YOUR INPUT`), HALTED_SILENT (`INTERVENTION REQUIRED` with native `title` tooltip), BLOCKED, plus additional `pipeline_status` values shipped in the UI (e.g. **COMPLETE** on lime `#2DEB1E` / `text-slate-900`, STOPPED, Queue stalled, IDLE, UNKNOWN). When `orchestrator_alive` is `false` and `pipeline_status` is `RUNNING` or `WAITING_FOR_SENTINEL`, status pill overrides to "ORCHESTRATOR DOWN" in red. Project path displays last two path segments in monospace. Liveness dot is visually green when `orchestrator_alive: true`, red when `false`. Header re-renders correctly on each state poll.
  > Notes: Polls `GET /api/state` every 3 seconds. Teal `run-pulse` applies only to RUNNING. WAITING_FOR_SENTINEL uses the same teal surface as RUNNING but **static** (no `run-pulse`). Amber/yellow remains for stall/issue-style pills (e.g. **Queue stalled**, queue **BLOCKED** / **ESCALATION**). The "ORCHESTRATOR DOWN" override takes precedence over any other label when the liveness condition is met. Full mapping: `docs/prd/AUTODEV-UI-PRD.md` § Header Bar.
...
- [x] `UI-4` | LOW | Current Phase: attempt counters, elapsed timer (no duplicate last-error chip or skill line under attempts)
  > Test: **Agent attempts** — heading above three rows; each row shows **three boxed cells** + neutral **`n/3`** (`computeAgentAttemptFractionN`); `getAgentAttemptDotStates` maps `planner_retries` / `executor_retries` / `reviewer_retries`, `current_agent`, and `pipeline_status` to per-slot visuals: **slate** pending (index digit), **blue** in-flight `#3b82f6` (`RUNNING` / `WAITING_FOR_SENTINEL` only; subtle pulse on in-flight cell; distinct from pipeline teal), **lime** success (`#2DEB1E`, check icon), **red** failure (`#dc2626`, X icon); native **`title`** on each cell. Escalation-with-exhausted-retries shows three red cells on the exhausted row; terminal statuses and `WAITING_FOR_HUMAN` freeze off the blue in-flight cell. No monospace **`last_error_code`** chip or **skill / agent** line under the attempts block (Activity feed: `gate_fail.detail.last_error_code` + H-23 summary; queue/metrics for skill). Elapsed timer increments live from `last_action_timestamp`; when `pipeline_status` is `WAITING_FOR_SENTINEL` and elapsed exceeds 5 minutes, timer text color changes to amber.
  > Notes: Elapsed timer uses client-side `setInterval` (1-second tick), not server push. Counter dots cap display at 3 (matching the 3-attempt budget per role). Attempt UI reads retries + timestamps from merged `GET /api/state`; gate error prose is not duplicated in the left strip (see `pipeline_events.jsonl` / feed).
...
- [x] `UI-7` | LOW | Implement the Activity Feed panel with static event row rendering, color-coded type badges, and inline row expansion
  > Test: Up to 30 events from `GET /api/events` render in reverse chronological order. Each row shows: timestamp (time portion only, monospace), **human-readable event type label** on the badge (mapped from jsonl `event` or synthetic `event_type`), **machine id** on badge **`title`** hover for grepping logs, agent, phase, attempt (or `—` if null), detail (truncated to ~60 chars). Badge colors match: `gate_pass` green tint, `gate_fail` and `retry` amber tint, `escalation_trigger` orange tint, `escalation_resolve` blue tint, `phase_complete` bright green, all others neutral gray. Clicking any row expands inline to show full detail text; clicking again collapses. When the event buffer is empty, a muted **"No events yet. Events appear here as the pipeline runs."** placeholder renders.
...
- [x] `UI-11` | LOW | Add confirmation modals for destructive commands and reset cap enforcement to the Escalation Command Panel
...
- [x] `UI-13` | LOW | Apply responsive vertical stacking for narrow viewports and add pulse animation to active status states
  > Test: At viewport width ≤ 768px, the layout stacks vertically in order: header → current phase panel → roadmap panel → activity feed. At viewport width > 768px, the two-column desktop layout is restored. Only the RUNNING status pill uses a visible, repeating CSS `run-pulse` animation (teal). WAITING_FOR_SENTINEL, WAITING_FOR_HUMAN, HALTED_SILENT, BLOCKED, and other pills have no animation (static solid color). New event rows in the Activity Feed fade in visibly over ~300ms. No other animations are present in the UI.
  > Notes: Responsive breakpoint via Tailwind `sm:` or a media query. Pipeline pulse: `teal-pulse` / `.run-pulse` on RUNNING only (`tests/test_ui_status_pulse_animation.py`). Feed fade-in: CSS transition on `opacity` from 0 to 1. These are the only two animation families permitted per the PRD Aesthetic Direction.

- [x] `INFRA-3` | LOW | Add `ui/autodev-ui.service` as a systemd unit file template for running the UI server on the Pi
