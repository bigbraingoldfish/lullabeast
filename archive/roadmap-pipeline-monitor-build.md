...
- [x] `UI-4` | LOW | Add attempt counters, last error code, elapsed timer, and skill injected display to the Current Phase panel
  > Test: **Agent attempts** — heading above three rows; each row maps `planner_retries`, `executor_retries`, or `reviewer_retries` (failure counts, max three visible slots) plus `current_agent` and `pipeline_status` to per-dot colors: **slate** = attempt not used yet, **teal** in-flight (`RUNNING` / `WAITING_FOR_SENTINEL` only; subtle pulse on in-flight slot), **lime** success (`#28D11B`, same as COMPLETE pill), **red** failure (`#dc2626`, same as BLOCKED/STOPPED pills); native **`title`** on each dot explains the state. Escalation-with-exhausted-retries shows three reds on the exhausted row; terminal statuses and `WAITING_FOR_HUMAN` freeze off the teal in-flight dot. `last_error_code` renders in monospace only when the field is present in the state response — absent or null means the field is not shown. Elapsed timer increments live in the browser from `last_action_timestamp`; when `pipeline_status` is `WAITING_FOR_SENTINEL` and elapsed exceeds 5 minutes, timer text color changes to amber. Skill injected renders as "discipline / agent" in small muted text when both `skill_injected` (non-null) and `skill_agent` are present in state; hidden otherwise.
