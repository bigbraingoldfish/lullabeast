...
- [ ] `API-1` | HIGH | Add `GET /api/state` returning merged `pipeline_state.json` and `phase_state.json` with server-derived liveness and event source fields
...
- [ ] `API-2` | LOW | Add `GET /api/roadmap` returning the parsed roadmap with the in-progress phase correctly identified
...
- [ ] `CORE-2` | HIGH | Implement the background state polling loop and in-memory synthetic event ring buffer
...
- [ ] `API-3` | LOW | Add `GET /api/events?limit=N&offset=M` serving the synthetic ring buffer with file-source fallback
...
- [ ] `API-5` | HIGH | Add `POST /api/command` that writes escalation files to `project_dir_path` after passing all validation checks
...
- [ ] `UI-2` | LOW | Implement the header bar showing the AUTODEV wordmark, pipeline status pill, project path, and liveness dot
...
- [ ] `UI-4` | LOW | Add attempt counters, last error code, elapsed timer, and skill injected display to the Current Phase panel
...
- [ ] `UI-5` | LOW | Implement the Roadmap panel showing all phases as a scrollable list with status icons, current phase highlight, and phase count progress bar
...
- [ ] `UI-8` | HIGH | Connect the Activity Feed to the SSE stream so new events appear at the top with a fade-in animation, with polling fallback when SSE is unavailable
...
- [ ] `UI-9` | LOW | Add an Escalation Log tab within the Activity Feed panel showing paired escalation events
