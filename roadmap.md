...
- [ ] `INFRA-E1` | HIGH | Extend init-project skill to cover all repo_init_check.py requirements and validate end-to-end
  > Test: Use `/tmp/infra-e1-test-a` (Mode A) and `/tmp/infra-e1-test-b` (Mode B) as the fixed test paths. Run the extended skill against each. After Mode A: (1) `~/.openclaw/pipeline-project` symlink was set to `/tmp/infra-e1-test-a` during the skill run — verify it resolves there via `readlink -f`; (2) project root contains `roadmap.md` that passes the format validator; (3) project root `.gitignore` contains all 7 required pipeline entries; (4) `gate_scripts/repo_init_check.py` runs against the result and exits 0. After Mode B: same checks pass, no existing files overwritten. **CRITICAL — after both modes pass**: immediately restore the production symlink `ln -sfn /home/pi/projects/autodev-ui ~/.openclaw/pipeline-project` and verify `readlink -f ~/.openclaw/pipeline-project` = `/home/pi/projects/autodev-ui` before committing. Failure to restore will break the orchestrator.
